...
export class StudentsService {
  ...
  async create(data: CreateStudentDto): Promise<StudentSummary> {
    const student = await this.studentsRepository.save(
      this.studentsRepository.create({ name: data.name, email: data.email || null }),
    );
  ...
  async addGrade(studentId: number, data: CreateGradeDto): Promise<StudentSummary> {
    const student = await this.getStudentEntity(studentId);
    await this.gradesRepository.save(this.gradesRepository.create({ ...data, student }));
  ...
  async removeGrade(studentId: number, gradeId: number): Promise<StudentSummary> {
    const grade = await this.gradesRepository.findOne({
      where: { id: gradeId, student: { id: studentId } },
    });
    if (!grade) throw new NotFoundException('Nota não encontrada.');
  ...
  private toSummary(student: Student): StudentSummary {
    const grades = [...(student.grades || [])].sort((a, b) => a.subject.localeCompare(b.subject));
    const average = grades.length
      ? Number((grades.reduce((total, grade) => total + grade.score, 0) / grades.length).toFixed(2))
      : null;
